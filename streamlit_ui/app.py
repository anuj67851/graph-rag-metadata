...
import tempfile
import os
import textwrap  # <--- CHANGE 1: Import textwrap
...
def display_pyvis_graph(graph_data: dict, graph_id: str) -> None:
    ...
    for node_info in graph_data.get("nodes", []):
        ...
        if properties.get("contexts"):
            full_context_str = f"Contexts: {', '.join(properties['contexts'][:3])}"
            wrapped_context = '\n'.join(textwrap.wrap(full_context_str, width=80))
            title_parts.append(wrapped_context)
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".html", prefix=f"{graph_id}_") as tmp_file:
            ...
            with open(tmp_file.name, "r", encoding="utf-8") as html_file:
                st.components.v1.html(html_file.read(), height=750, scrolling=True) # Increased height slightly
...
with tab1_ingest:
    ...
    st.markdown(
        "Upload PDF, TXT, DOCX, or Markdown files. The system will create semantic chunks, extract information, and build the knowledge base.")
    ...
    if uploaded_files:
        if st.button("Process Uploaded Files"):
            ...
            with st.spinner(
                    "Processing files... This may take time (semantic chunking requires multiple LLM calls per document)."):
                try:
                ...
                if "source_chunks" in message and message["source_chunks"]:
                    with st.expander("Show Retrieved Text Chunks"):
                        for chunk in message["source_chunks"]:
                            st.info(
                                f"**From: {chunk['source_document']} (Score: {chunk['score']:.4f})**\n\n> {chunk['chunk_text'].replace('  ', ' ')}")
...
with tab3_explore:
    ...
    explore_option = st.selectbox("Choose an exploration option:",
    ...
    if explore_option == "View Full Graph Sample":
        ...
        if st.button("Load Full Graph Sample"):
            with st.spinner("Loading graph sample..."):
                try:
                    response = api_request("GET", "/graph/full_sample",
                                           params={"node_limit": node_limit, "edge_limit": edge_limit})
                    graph_data = response.json()
                    st.success(
                        f"Graph sample loaded: {len(graph_data.get('nodes', []))} nodes, {len(graph_data.get('edges', []))} edges.")
        ...
        if st.button("Load Busiest Nodes"):
            with st.spinner(f"Loading top {top_n} busiest nodes and their neighborhood..."):
                try:
                    ...
                    graph_data = response.json()
                    st.success(
                        f"Busiest nodes subgraph loaded: {len(graph_data.get('nodes', []))} nodes, {len(graph_data.get('edges', []))} edges.")
